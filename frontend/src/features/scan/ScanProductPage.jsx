...
const StatusIcon = ({ type }) => {
  ...
  return <HelpIcon {...props} sx={{ ...props.sx, color: 'text.secondary' }} />;
};
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, fontFamily: 'Inter, sans-serif' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3 }}>
        <Box>
          <Typography sx={{ fontWeight: 700, fontSize: '1.75rem', color: 'text.primary', mb: 0.5 }}>Product Analysis</Typography>
          <Typography sx={{ color: 'text.secondary', fontSize: '0.9375rem' }}>Initiate real-time scan or manual entry for clinical safety verification.</Typography>
        </Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 300px' }, gap: 3 }}>
          ...
          <Card elevation={0} sx={{ border: '1px solid #e5e7eb', borderRadius: '14px', overflow: 'hidden' }}>
            <Box sx={{ px: 2.5, py: 1.75, borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <QrCodeScannerIcon sx={{ fontSize: 18, color: 'text.primary' }} />
                <Typography sx={{ fontWeight: 600, fontSize: '0.9375rem', color: 'text.primary' }}>Live Scanner View</Typography>
              </Box>
          ...
          <Card elevation={0} sx={{ border: '1px solid #e5e7eb', borderRadius: '14px' }}>
            <CardContent sx={{ p: 2.5 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                <QrCodeScannerIcon sx={{ fontSize: 16, color: 'text.primary' }} />
                <Typography sx={{ fontWeight: 600, fontSize: '0.9375rem', color: 'text.primary' }}>Manual Code Entry</Typography>
              </Box>
        ...
        <Box>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
            <Typography sx={{ fontWeight: 700, fontSize: '1.125rem', color: 'text.primary' }}>Recent Scans</Typography>
                ...
                <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography sx={{ fontWeight: 600, fontSize: '0.875rem', color: 'text.primary', mb: 0.25 }} noWrap>{scan.name}</Typography>
                      <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary', mb: 0.75 }} noWrap>UPC: {scan.upc}</Typography>
